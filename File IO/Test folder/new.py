...
class Ghost(Player):
    ...
    def changespeed(self,list,ghost,turn,steps,l):
        if turn == True:
            if steps == 0:
                self.change_x = list[l][0]
                self.change_y = list[l][1]
                steps = steps + 1
                l = l + 1
            elif steps == 1:
                self.change_x = list[l][0]
 ...
 # CHANGE - Add timer for doubling the ghosts 
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_LEFT:
                    Pacman.changespeed(-1,0)
                elif event.key == pygame.K_RIGHT:
                    Pacman.changespeed(1,0)
                elif event.key == pygame.K_UP:
                    Pacman.changespeed(0,-1)
...
def duplicate_ghosts(all_sprites_list,monsta_list):
    for monsta in monsta_list:
        monsta.duplicate(all_sprites_list)
